...
int main( int argc, char *argv[] )
{
    ...
    if(settings.flag_method[0]== "POD_STEADY") {
        ...
        if (settings.r == 0) {
            ...
            std::cout << "Number of modes for desired energetic content: " << Nrec << std::endl;
        } else {
            int max_rank = Phi.cols();
            Nrec = std::min(settings.r, max_rank);
            std::cout << "Number of modes (fixed): " << Nrec << std::endl;
        }

        if (settings.flag_rec == "YES") {
            Eigen::VectorXd Err_SPOD_Nm_time = Eigen::VectorXd::Zero(settings.alpha_rec.size());
            for (int nt = 0; nt < settings.alpha_rec.size(); nt++) {
                std::cout << "Reconstructing field for   alpha: " << settings.alpha_rec[nt] << "\t and Mach: "
                          << settings.mach_rec[nt] << std::endl;
                Eigen::MatrixXd Rec = Reconstruction_steady_POD(param,
                                                                K_pc, lambda, eig_vec.transpose(),
                                                                Phi, settings.alpha_rec[nt], settings.mach_rec[nt],
                                                                Nrec,
                                                                settings.flag_prob,
                ...
                std::cout << "Computing error of interpolation and error from projection..." << "\t" << std::endl;
                Eigen::VectorXd Err_SPOD_map = Eigen::VectorXd::Zero(sn_set.rows());
                //Eigen::VectorXd Err_SPOD_Nm_time = Eigen::VectorXd::Zero(settings.Ns);


                Err_SPOD_map = sn_set.col(nt) - Rec;
                ...
                for (int j = 0; j < Nr; j++) {
                    Err_SPOD_Nm_time(nt) += Err_SPOD_map(j) * Err_SPOD_map(j);
                }

                Err_SPOD_Nm_time(nt) = std::sqrt(Err_SPOD_Nm_time(nt)) / norm_sn_set(nt);
                std::cout << "Done" << std::endl;

            }
            std::cout << Err_SPOD_Nm_time << std::endl;
            ...
            std::cout << "Writing error of interpolation  on a file " << std::endl;

            std::ofstream errfile;
            errfile.open("Err_POD_steady.dat");
            for (int j = 0; j < Err_SPOD_Nm_time.rows(); j++) {
                errfile << param.row(j) << '\t'<<'\t';
                errfile << std::setprecision(8) << Err_SPOD_Nm_time(j) << std::endl;
            }
