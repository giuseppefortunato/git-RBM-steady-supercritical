    ...
    if ( cFile.is_open() )
    {
        ...
        while( getline(cFile, line) )
        {
            ...
            switch (read_keyword_type(name))
            {
                ...
                case R_ISOMAP:
                {
                    settings.r_isomap = std::stoi(value);
                    //std::cout << "DMD rank : " << value << std::endl;
                    break;
                }
                ...
                case ALPHA_REC:
                {
                    ...
                    while ( ss >> i ) {
                        ...
                        settings.alpha_rec.push_back(i);
...
void Modify_su2_cfg( std::string file_in, std::string file_out,std::string filename, prob_settings settings, int nt) {
    ...
    if ( inFile.is_open() && outFile.is_open() ) {
        ...
        if(settings.analysis=="STEADY") {
            std::cout<<"CONTROL THE MATCH BETWEEN THE PARAMETERS AND THE MODIFICTAION OF SU2 FILE"<<std::endl;
            while (getline(inFile, line)) {
                ...
                if (name ==
                    "MACH_NUMBER") {                                                                      //commented just for supercritical
                    ...
                    outFile << "RESTART_FILENAME=" << filename;

                }else if (name == "CONV_FILENAME"){
                    if(settings.flag_method[0]=="POD")
                        outFile << "CONV_FILENAME=" << "history_"+ settings.flag_method[0]+"_"+std::to_string(settings.r) +"_"+std::to_string(nt+1);
                    else if(settings.flag_method[0]=="ISOMAP")
                        outFile << "CONV_FILENAME=" << "history_"+ settings.flag_method[0]+"_"+std::to_string(settings.r_isomap) +"_"+std::to_string(nt+1);

                }else if (name == "VOLUME_FILENAME"){
                    if (settings.flag_method[0]=="POD")
                        outFile << "VOLUME_FILENAME=" << "VOL_"+ settings.flag_method[0]+"_"+std::to_string(settings.r) +"_"+std::to_string(nt+1);
                    else if (settings.flag_method[0]=="ISOMAP")
                        outFile << "VOLUME_FILENAME=" << "VOL_"+ settings.flag_method[0]+"_"+std::to_string(settings.r_isomap) +"_"+std::to_string(nt+1);

                }else if (name == "SURFACE_FILENAME"){
                    if (settings.flag_method[0]=="POD")
                        outFile << "SURFACE_FILENAME=" << "SURF_"+ settings.flag_method[0]+"_"+ std::to_string(settings.r) +"_"+std::to_string(nt+1);
                    else if(settings.flag_method[0]=="ISOMAP")
                        outFile << "SURFACE_FILENAME=" << "SURF_"+ settings.flag_method[0]+"_"+ std::to_string(settings.r_isomap) +"_"+std::to_string(nt+1);
